...
import {
  fetchPlanesMantenimientoSnapshot,
  filterPlanRecords,
} from "@/lib/planes-mantenimiento/planes-service";

const QUERY_KEY = ["planes-mantenimiento", "snapshot"] as const;
...
export function usePlanesMantenimiento() {
  ...
  const { data, isLoading, isFetching, refetch, dataUpdatedAt } = useQuery({
    queryKey: QUERY_KEY,
    queryFn: fetchPlanesMantenimientoSnapshot,
    staleTime: 30_000,
  });

  const filteredRecords = useMemo(() => {
    if (!data) return [];
    return filterPlanRecords(data.records, { activeTab, search });
