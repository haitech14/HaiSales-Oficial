...
import {
  fetchCajaChicaSnapshot,
  filterCajaChicaRecords,
} from "@/lib/caja-chica/caja-chica-service";

const QUERY_KEY = ["caja-chica", "snapshot"] as const;
...
export function useCajaChica() {
  ...
  const { data, isLoading, isFetching, refetch, dataUpdatedAt } = useQuery({
    queryKey: QUERY_KEY,
    queryFn: fetchCajaChicaSnapshot,
    staleTime: 30_000,
  });

  const filteredRecords = useMemo(() => {
    if (!data) return [];
    return filterCajaChicaRecords(data.records, { activeTab, search });
