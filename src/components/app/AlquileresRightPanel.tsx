...
import { formatAlquilerCurrency } from "@/lib/alquileres/alquileres-mock-data";
import type { AlquileresSnapshot } from "@/lib/alquileres/alquileres-service";
...
function StatusDonutChart({
  ...
  segments: AlquileresSnapshot["statusSegments"];
  total: number;
}) {
  let offset = 0;
...
type AlquileresRightPanelProps = {
  className?: string;
  snapshot?: AlquileresSnapshot | null;
  lastUpdatedAt: Date | null;
  onRefresh: () => void;
  isRefreshing: boolean;
};

export function AlquileresRightPanel({
  className,
  snapshot,
  lastUpdatedAt,
  onRefresh,
  isRefreshing,
}: AlquileresRightPanelProps) {
  const total = snapshot?.totalRecords ?? 0;
  const vencimientos = snapshot?.vencimientosProximos ?? [];
  const equipos = snapshot?.equiposRotacion ?? [];

  return (
    <aside className={cn("w-[300px] shrink-0 border-l border-slate-200 bg-white", className)}>
      <div className="space-y-5 p-4">
        <section>
          <h3 className="app-panel-title">Alquileres por estado</h3>
          <div className="mt-4">
            <StatusDonutChart segments={snapshot?.statusSegments ?? []} total={total} />
          </div>
        </section>

        <section className="border-t border-slate-100 pt-4">
          <h3 className="app-panel-title">Vencimientos próximos</h3>
              ...
              <li key={item.label}>
                <div className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className="shrink-0 text-right font-semibold text-slate-900">
                    {item.contracts} contrato{item.contracts === 1 ? "" : "s"}
                    <span className="ml-1 font-normal text-slate-500">
                      {formatAlquilerCurrency(item.amount)}
                    </span>
                  </span>
                </div>
        ...
        <section className="border-t border-slate-100 pt-4">
          <h3 className="app-panel-title">Equipos con mayor rotación</h3>
          <ul className="mt-3 space-y-3">
            {equipos.map((item) => (
              <li key={item.name}>
                <div className="flex items-center justify-between gap-2 text-xs">
