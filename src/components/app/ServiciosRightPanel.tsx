...
import type { ServiciosSnapshot } from "@/lib/servicios/servicios-service";
...
function EstadoDonutChart({
  ...
  segments: ServiciosSnapshot["estadoSegments"];
  total: number;
}) {
  let offset = 0;
...
type ServiciosRightPanelProps = {
  className?: string;
  snapshot?: ServiciosSnapshot | null;
  lastUpdatedAt: Date | null;
  onRefresh: () => void;
  isRefreshing: boolean;
};

export function ServiciosRightPanel({
  className,
  snapshot,
  lastUpdatedAt,
  onRefresh,
  isRefreshing,
}: ServiciosRightPanelProps) {
  const total = snapshot?.totalRecords ?? 0;
  const antiguedad = snapshot?.antiguedadOrdenes ?? [];
  const tecnicos = snapshot?.tecnicosTop ?? [];

  return (
    <aside className={cn("w-[300px] shrink-0 border-l border-slate-200 bg-white", className)}>
      <div className="space-y-5 p-4">
        <section>
          <h3 className="app-panel-title">Servicios por estado</h3>
          <div className="mt-4">
            <EstadoDonutChart segments={snapshot?.estadoSegments ?? []} total={total} />
          </div>
        </section>

        <section className="border-t border-slate-100 pt-4">
          <h3 className="app-panel-title">Antigüedad de órdenes</h3>
        ...
        <section className="border-t border-slate-100 pt-4">
          <h3 className="app-panel-title">Técnicos con más atenciones</h3>
          <ul className="mt-3 space-y-3">
            {tecnicos.map((item) => (
              <li key={item.name}>
                <div className="flex items-center justify-between gap-2 text-xs">
