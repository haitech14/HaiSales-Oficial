...
import {
  buildVentasPorTipoRows,
  buildVentasResumen,
  formatVentasSoles,
  type VentasResumenMoneda,
} from "@/lib/ventas/ventas-page-utils";
import { cn } from "@/lib/utils";

const MONEDA_TABS: Array<{ id: VentasResumenMoneda; label: string }> = [
...
type VentasResumenDiaPanelProps = {
  records: VentaRecord[];
  className?: string;
};

export function VentasResumenDiaPanel({ records, className }: VentasResumenDiaPanelProps) {
  const [moneda, setMoneda] = useState<VentasResumenMoneda>("general");
  const resumen = buildVentasResumen(records);
  const porTipoRows = buildVentasPorTipoRows(records);
    ...
    >
      ...
      <div className="space-y-4 px-5 py-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <ResumenMetricCard
            label="Emisiones"
            value={String(resumen.emisiones)}
            icon={FileText}
            tone="blue"
          />
          <ResumenMetricCard
      ...
      <div className="mt-auto border-t border-slate-100 px-5 py-4">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span>
            Última emisión <strong className="font-bold text-slate-800">{resumen.ultimaEmision}</strong>
          </span>
        </div>
      </div>
    </aside>
