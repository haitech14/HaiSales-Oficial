...
import { CajaChicaRightPanel } from "@/components/app/CajaChicaRightPanel";
...
} from "@/lib/caja-chica/caja-chica-service";
import { cn } from "@/lib/utils";

const PAGE_SIZE_OPTIONS = [10, 25, 50];

type CajaChicaPageProps = {
  embedded?: boolean;
  rightPanel?: ReturnType<typeof useAppRightPanel>;
};

export default function CajaChicaPage({ embedded = false, rightPanel }: CajaChicaPageProps) {
  const {
    snapshot,
    paginatedRecords,
    filteredRecords,
  ...
  return (
    <div className={cn("flex flex-col", embedded ? "min-h-0 flex-1" : "min-h-screen")}>
      {!embedded && (
        <AppPageHeader
          title="Caja Chica"
          subtitle="Controla fondos, solicitudes, rendiciones, comprobantes y saldos por responsable."
          showPanelToggle
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-4 py-3">
                <div className="relative min-w-[220px] flex-1">
                  ...
                  <input
                    ...
                    placeholder="Buscar por responsable, concepto o comprobante..."
                        ...
                        >
                          ...
                          <td className="app-table-cell">
                            <span
                              className={cn(
                                "app-table-badge inline-flex rounded-full border",
                                getCajaChicaEstadoStyles(item.status),
                              )}
                            >
                              {item.status}
                            </span>
                          ...
                          >
                            {formatCajaChicaAmount(item.amount)}
                          </td>
              ...
              <div className="app-pagination-bar flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-4 py-2.5">
                <p>
                  {filteredCount === 0
                    ...
                    : `Mostrando ${startIndex.toLocaleString("es-PE")} a ${endIndex.toLocaleString("es-PE")} de ${filteredCount.toLocaleString("es-PE")} registros`}
                </p>
