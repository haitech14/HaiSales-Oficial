-- Sync WhatsApp inbox conversations into Pipeline "Prospectos"
...
BEGIN
  ...
  WITH wa_leads AS (
    SELECT
      ...
      COALESCE(NULLIF(btrim(conn.display_name), ''), 'WhatsApp') AS responsable_nombre
    FROM public.inbox_conversations c
    LEFT JOIN public.inbox_channel_connections conn ON conn.id = c.connection_id
    WHERE c.user_id = p_user_id
      AND c.channel = 'whatsapp'
      AND COALESCE(c.stage, 'nuevo') IN ('nuevo', 'seguimiento')
