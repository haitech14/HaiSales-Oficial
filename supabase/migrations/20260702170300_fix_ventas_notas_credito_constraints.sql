-- Notas de crédito y legacy import: permitir montos negativos y coherencia subtotal+igv=total

ALTER TABLE public.ventas DROP CONSTRAINT IF EXISTS ventas_subtotal_check;
ALTER TABLE public.ventas DROP CONSTRAINT IF EXISTS ventas_igv_check;
ALTER TABLE public.ventas DROP CONSTRAINT IF EXISTS ventas_total_check;

ALTER TABLE public.ventas
  ADD CONSTRAINT ventas_totales_check
  CHECK (abs(total - (subtotal + igv)) <= 0.02);

ALTER TABLE public.venta_items DROP CONSTRAINT IF EXISTS venta_items_cantidad_check;
ALTER TABLE public.venta_items DROP CONSTRAINT IF EXISTS venta_items_subtotal_check;

ALTER TABLE public.venta_items
  ADD CONSTRAINT venta_items_cantidad_check
  CHECK (cantidad <> 0);
