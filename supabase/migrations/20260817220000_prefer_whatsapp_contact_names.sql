-- Prefer WhatsApp saved name / username over a phone number when syncing Prospectos.
...
BEGIN
  ...
  WITH wa_leads AS (
    SELECT
      ...
      COALESCE(
        NULLIF(btrim(c.metadata->>'display_name'), ''),
        NULLIF(btrim(c.metadata->>'profile_name'), ''),
        NULLIF(btrim(c.metadata->>'wa_username'), ''),
        NULLIF(btrim(c.contact_name), ''),
        'Contacto WhatsApp'
      ) AS cliente_nombre,
      COALESCE(c.contact_identifier, '') AS telefono,
      LEFT(COALESCE(NULLIF(btrim(c.last_message), ''), 'Conversacion WhatsApp'), 160) AS subtitulo,
  ...
  upserted AS (
    ...
    ON CONFLICT (user_id, codigo) DO UPDATE
      SET
        cliente_nombre = CASE
          WHEN length(regexp_replace(btrim(EXCLUDED.cliente_nombre), '\D', '', 'g')) >= 7
               AND length(regexp_replace(btrim(EXCLUDED.cliente_nombre), '\D', '', 'g'))::numeric
                   / GREATEST(length(regexp_replace(btrim(EXCLUDED.cliente_nombre), '\s', '', 'g')), 1) >= 0.7
               AND public.oportunidades.cliente_nombre ~ '[A-Za-zÁÉÍÓÚÜáéíóúüÑñ]'
          THEN public.oportunidades.cliente_nombre
          ELSE EXCLUDED.cliente_nombre
        END,
